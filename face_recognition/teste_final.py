...
class FaceRecognitionService(Node):
    ...
    def asr_cb(self, msg):        
        if self.use_asr:
            ...
            self.words.append(word for word in msg.data.split(" "))
            self.words.append(msg.data)
        else:
            self.get_logger().info('asr')
    ...
    def image_cb(self, msg: Image):
        ...
        if self.mode == 'generate':
            if len(embedding_objs) == 1:
                if self.embedding_alvo != None:
                    ...
                    self.get_logger().info(f'{self.use_asr}')
        ...
        # Compara os embeddings do rosto alvo com a multidão
            ...
            publishing_nav.data = 'gira essa buceta'
            self._pub_nav.publish(publishing_nav)

            self.get_logger().error('Iniciando comparacao.')
