...
def publish_message(publisher, msg_data, msg_type):
    if msg_type == "string":
        ...
        # msg_data já deve ser um objeto Image pronto
        if not isinstance(msg_data, Image):
            raise TypeError("Para msg_type='image', msg_data deve ser um objeto do tipo sensor_msgs.msg.Image")
        msg = msg_data
    else:
...
def main(args=None):
    ...
    log_publisher = node.create_publisher(Image, 'log_image', 10)
    ...
    names_list = ["Miguel", "Arthur", "Artur", "Heitor", "Eitor", "Davi", "Gabriel", "Bernardo", "Gael", "Enzo Gabriel", "Samuel", "Theo", "Teo", "Alice", "Helena", "Laura", "Maria Alice", "Maria Eduarda", "Maria Clara", "Valentina", "Maria Cecília", "Maria Julia", "Heloisa", "Eloisa"]
    ...
    # Ordena a lista de nomes do maior para o menor (em nº de caracteres)
    names_list_sorted = sorted(names_list, key=len, reverse=True)
    ...
    while True:
        ...
        if not found_name:
            found_name = next((name for name in names_list_sorted if name in msg.data), None)
            if found_name:
    ...
    timer = time.time()
    # save log with timestamp HH:MM:SS
    timestamp = time.strftime("%H%M%S", time.localtime(timer))
    cv2.imwrite(f'face_recognition/log_image{timestamp}.jpg', cv2.cvtColor(log_image, cv2.COLOR_BGR2RGB))
    ...
    change_face_display("cam")

    # converte para mensagem ROS2 do tipo sensor_msgs/Image
    image_msg = cv_bridge.cv2_to_imgmsg(cv_image, encoding="bgr8")
    # publica no tópico
    publish_message(log_publisher, image_msg, "image")
