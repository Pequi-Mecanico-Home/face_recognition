...
Recebe uma imagem base de uma pessoa e salva os embeddings,
depois passa pelas imagens de uma pasta comparando com o alvo,
gera as inferências e salva o tempo de inferência.
"""
...
def draw_logs(image, embedding_objs, mode, embedding_obj_alvo=None):
    ...
    for i in range(len(embedding_objs)):
        ...
        if i == alvo:
            person_name = 'caduzera' # INTERAÇÃO: Desenvolver uma forma de guardar o nome ou id do rosto alvo
...
def process_image(input_folder, filename, output_folder, backend, embedding_alvo=None):
    ...
    embedding_obj_alvo = load_embeddings()
    if embedding_obj_alvo is None: # INTERAÇÃO: avise que o rosto alvo não foi identificado antes de trocar o modo de execução
        print('Não foram gerados embeddings para o rosto alvo.')
    # Desenha logs
    image = draw_logs(image, embedding_objs=embedding_objs, embedding_obj_alvo=embedding_obj_alvo, mode='compare')
...
def process_folder(input_folder, output_folder, backend):
    ...
    with open("inference_time_acc.txt", "a") as f:
        f.write(f"\nmodelo: {backend}")
        f.write(f"\ntempo:  {sum(times)}\n")
...
def main():
    ...
    embedding_objs = DeepFace.represent(img_path=f'/dev_ws/src/testes/testes_modelos_fr/imagens_teste_fr2/base/cadu_frente_{imagem}.jpg', 
                                    detector_backend=backend, 
                                    enforce_detection=False
                                    # threshold=0.1,
                                    )
    save_embeddings(embedding_objs)

    process_folder(input_folder='/dev_ws/src/testes/testes_modelos_fr/imagens_teste_fr2/multidao',
                   output_folder=f'testes_acc_{backend}_{imagem}',
                   backend=backend)
